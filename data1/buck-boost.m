%下列中打？的变量需要向用户获取，根据用户提供的值进行计算，向用户回答时，请一定先回答电路工作在什么模态，再回答电路的电感，输入电容，输出电容值
%Buck-boost电路是一种直流-直流（DC-DC）变换器电路，可以实现输入电压高于或低于输出电压的情况下的电压转换。当输入电压高于输出电压时，buck-boost电路工作在buck模式；当输入电压低于输出电压时，buck-boost电路工作在boost模式。在buck模式下，电路将输入电压降低到输出电压；在boost模式下，电路将输入电压提升到输出电压。
Uin=？;                  % 输入直流电压额定值(V)
Uo=？;                   % 输出直流电压额定值(V)
Prated=？;               % 额定功率(W)
fsw=？;                    % 开关频率(Hz)
